<!--
The skeleton `prod` produces at PRODUCT.md — the durable "who, what, why" that
`arch` and `spec` build on. Keep it terse and scannable; capture intent and constraints,
not a roadmap. Omit a section only if it genuinely doesn't apply. Link out rather than restate.
-->

# Product

**Status:** Draft | Current
**Updated:** YYYY-MM-DD

## Purpose

The problem and why it matters now — one or two lines. Motivation, not the solution.

## Users

Who has the problem and the context they're in. Primary user first; note secondary users only if they change the product.

## Solution

A product concept we believe solves the problem.
The north star that holds even as features change. Aspirational, concise, not a feature list.

## Value

The core jobs the product does for those users — what they can do that they couldn't before. A short list.

## Competition

How the problem is solved today — competitors or workarounds — and why ours wins, one line each. Omit for internal tools.

## Constraints

The hard limits that shape the build: timeline, compliance/regulatory, accessibility, and any explicit non-negotiables.

## Non-functional targets

Only what's known and business-driven: performance, availability/SLA, scale, required integrations. Don't invent numbers; `arch` translates these into architecture drivers.

## Scope

### Core Features (MVP)

What ships first to deliver the core value.

### Out / Later

What is explicitly deferred.

## Success signals

How we'll know it works — observable outcomes or metrics, not vanity numbers.

## References

Link related materials — briefs, research, design (`DESIGN.md`), prior art.
